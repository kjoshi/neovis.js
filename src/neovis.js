...
export default class NeoVis {
            ...
            for (let key of Object.keys(config.labels)) {
                ...
                config = {
                    ...
                    labels: {
                        ...config.labels,
                        [key]: {
                            ...config.labels[NEOVIS_DEFAULT_CONFIG],
                            ...config.labels[key]
                        }
                    }
                };
            ...
            for (let key of Object.keys(config.relationships)) {
                config = {
                    ...
                    relationships: {
                        ...
                        [key]: {
                            ...config.relationships[NEOVIS_DEFAULT_CONFIG],
    ...
    async buildNodeVisObject(neo4jNode) {
        ...
        let labelConfig =
            this._config &&
            this._config.labels &&
            (this._config.labels[label] || this._config.labels[NEOVIS_DEFAULT_CONFIG]);
        ...
        if (typeof captionKey === 'function') {
            ...
            captionKey &&
            (typeof neo4jNode.properties[captionKey] === 'number' || Neo4j.isInt(neo4jNode.properties[captionKey]))
        ) {
    ...
    render(query) {
        ...
        session.run(_query, { limit: 30 }).subscribe({
            onNext: (record) => {
                recordCount++;
            ...
            onCompleted: async () => {
                ...
                this._network.on('click', function (params) {
                    if (params.nodes.length > 0) {
                        neoVis._highlightActive = true;
                        const selectedNode = params.nodes[0];
                        // Set all nodes to grey
                        Object.keys(neoVis._nodes).map((nodeId) => {
                            neoVis._nodes[nodeId].color = 'rgba(200,200,200,0.5)';
                            if (neoVis._nodes[nodeId].hiddenLabel === undefined) {
                                neoVis._nodes[nodeId].hiddenLabel = neoVis._nodes[nodeId].label;
                                neoVis._nodes[nodeId].label = undefined;
                            }
                        });

                        let connectedNodes = neoVis._network.getConnectedNodes(selectedNode);
                        ...
                        for (i = 1; i < 2; i++) {
                            for (j = 0; j < connectedNodes.length; j++) {
                                allConnectedNodes = allConnectedNodes.concat(
                                    neoVis._network.getConnectedNodes(connectedNodes[j])
                                );
                            }
                        }
                        ...
                        for (i = 0; i < allConnectedNodes.length; i++) {
                            neoVis._nodes[allConnectedNodes[i]].color = 'rgba(150,150,150,0.75)';
                            if (neoVis._nodes[allConnectedNodes[i]].hiddenLabel !== undefined) {
                                neoVis._nodes[allConnectedNodes[i]].label =
                                    neoVis._nodes[allConnectedNodes[i]].hiddenLabel;
                                neoVis._nodes[allConnectedNodes[i]].hiddenLabel = undefined;
                            }
                        }

                        // all first degree nodes get their own color and their label back
                        for (i = 0; i < connectedNodes.length; i++) {
                            neoVis._nodes[connectedNodes[i]].color = undefined;
                            if (neoVis._nodes[connectedNodes[i]].hiddenLabel !== undefined) {
                                neoVis._nodes[connectedNodes[i]].label = neoVis._nodes[connectedNodes[i]].hiddenLabel;
                                neoVis._nodes[connectedNodes[i]].hiddenLabel = undefined;
                            }
                        }

                        // Set the selected node back to its original colour
                        neoVis._nodes[selectedNode].color = undefined;
                        if (neoVis._nodes[selectedNode].hiddenLabel !== undefined) {
                            neoVis._nodes[selectedNode].label = neoVis._nodes[selectedNode].hiddenLabel;
                            neoVis._nodes[selectedNode].hiddenLabel = undefined;
                        }
                        neoVis._data.nodes.update(Object.values(neoVis._nodes));
                    } else if (neoVis._highlightActive) {
                        // Reset all the colours
                        neoVis._highlightActive = false;
                        Object.keys(neoVis._nodes).map((nodeId) => {
                            neoVis._nodes[nodeId].color = undefined;
                            if (neoVis._nodes[nodeId].hiddenLabel !== undefined) {
                                neoVis._nodes[nodeId].label = neoVis._nodes[nodeId].hiddenLabel;
                                neoVis._nodes[nodeId].hiddenLabel = undefined;
                            }
                        });
                        neoVis._data.nodes.update(Object.values(neoVis._nodes));
                    }
