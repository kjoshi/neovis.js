...
export default class NeoVis {
  ...
  async buildNodeVisObject(neo4jNode) {
    ...
    let labelConfig =
      this._config &&
      this._config.labels &&
      (this._config.labels[label] ||
        this._config.labels[NEOVIS_DEFAULT_CONFIG]);
    ...
    if (sizeCypher) {
      ...
      const session = this._driver.session(
        this._database && { database: this._database }
      );
      try {
        const result = await session.readTransaction((tx) =>
          tx.run(sizeCypher, { id: Neo4j.int(node.id) })
        );
      ...
      if (sizeProp && typeof sizeProp === "number") {
        ...
        sizeProp &&
        typeof sizeProp === "object" &&
        Neo4j.isInt(sizeProp)
      ) {
    ...
    if (typeof captionKey === "function") {
      ...
      captionKey &&
      (typeof neo4jNode.properties[captionKey] === "number" ||
        Neo4j.isInt(neo4jNode.properties[captionKey]))
    ) {
    ...
    if (!communityKey) {
      ...
      try {
        if (neo4jNode.properties[communityKey]) {
          node.group =
            neo4jNode.properties[communityKey].toNumber() || label || 0; // FIXME: cast to Integer
        } else {
          node.group = 0;
        }
      } catch (e) {
  ...
  buildEdgeVisObject(r) {
    const nodeTypeConfig =
      ...
      (this._config.relationships[r.type] ||
    ...
    if (typeof captionKey === "boolean") {
      ...
      if (
        typeof r.properties[captionKey] === "number" ||
        Neo4j.isInt(r.properties[captionKey])
      ) {
